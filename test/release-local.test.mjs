...
test("parses explicit and menu release selectors and rejects unsafe usage", async () => {
  ...
  assert.match(questions[0], /1\) notify[\s\S]*2\) permissions[\s\S]*3\) subagent[\s\S]*4\) status-line[\s\S]*5\) root[\s\S]*请输入选择：/u);
  for (const [answer, selector] of [["2", "permissions"], ["3", "subagent"], ["4", "status-line"], ["5", "root"], ["notify", "notify"], ["root", "root"]]) {
    assert.deepEqual(await resolveReleaseSelector([], {
      interactive: true,
      ask: async () => answer,
    }), {
...
test("delegates security-key authentication to interactive npm publish", async (t) => {
  const current = await state({ answers: ["y"] });
  t.after(current.cleanup);
  ...
  assert.equal(current.publishCalls.length, 1);
  assert.equal(current.publishOptions[0].interactive, true);
});
